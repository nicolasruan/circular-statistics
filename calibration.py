...
tests = [rayleigh, rayleigh2, watson, kuiper]
test_names = ["rayleigh", "rayleigh2", "watson", "kuiper"]
...
def plot_performance(table):
    ...
    fig, axs = plt.subplots(len(tests), len(table[0][0]), sharex = True, sharey = True)
    plt.suptitle("Calibration")
    X = np.linspace(20, max_size, steps)
    X_ticks = [20, 100, 200]
    if len(table[0][0])>1:
        for t in range(0, len(tests)):
            for i in range(0, len(table[0][0])):
                Y_i = []
                for j in range(0, steps):
                    Y_i.append(table[t][j][i])
                axs[t][i].plot(X, Y_i)
                axs[t][i].set_ylim(ymin = 0, ymax = 1.05)
                axs[t][i].label_outer()
                #if i == 0:
        ...
        for t in range(0, len(tests)):
            ...
            axs[t].plot(X, Y_i)
            axs[t].set_ylim(ymin=0, ymax = 0.15)
            axs[t].set_xticks(X, True)
            axs[t].set_xticks(X_ticks, False)
            axs[t].hlines(0.05, 20, max_size, colors="r")
            axs[t].label_outer()
